...
class relu_monom_coeffs():
    ...
    def get_quadratic_coeffs_over(self, interval):
    ##################################### perform linear programming #############################################################

        b0 = self.relu(interval[0])
        b1 = 0.5 * (self.relu(interval[0]) + self.relu(interval[1]))
        b2 = self.relu(interval[1])

        a0 = b0   +   (interval[0] / ( interval[0] - interval[1] ) ) * ( -2*b0 + 2*b1  ) + (interval[0] / ( interval[1] - interval[0] ) ) ** 2  * ( b0 - 2*b1 + b2  ) 
        a1 =          (1 / ( interval[0] - interval[1] ) ) * ( -2*b0 + 2*b1  ) - ( 2 * interval[0] / ( interval[1] - interval[0] ) )   * ( b0 - 2*b1 + b2  ) 
        a2 =           (1 / ( interval[1] - interval[0] ) ) ** 2  * ( b0 - 2*b1 + b2  ) 
    ...
    def get_quadratic_coeffs_under(self, interval):
    ##################################### perform linear programming #############################################################
        ...
        dmin = interval[0].item()
        dmax = interval[1].item()


        # declare coefficients of the objective function 
        # c =  [ ((dmin ** 3) / 3) - ((dmax ** 3) / 3) ,  ((dmin ** 2) / 2) - ((dmax ** 2) / 2), dmin - dmax]
        c = [ ((dmin ** 3) / 3) - ((dmax ** 3) / 3) ,  ((dmin ** 2) / 2) - ((dmax ** 2) / 2)]
        ...
        b = [0, dmax, 0, 0]
        # solve 
        # results = linprog(c = c, A_ub = A, b_ub = b, bounds = [x1_bounds, x2_bounds, x3_bounds])
        results = linprog(c = c, A_ub = A, b_ub = b, bounds = [x1_bounds, x2_bounds])

        coeffs = [0, results.x[1], results.x[0]]
    ...
    def get_quadratic_coeffs_under_mod(self, interval):
    ##################################### perform linear programming #############################################################
        ...
        f_v_1 = 0
        f_v_2 = (1 / 6) * (2 * dmax ** 2 - dmax * dmin - dmin ** 2)
        f_v_3 = 0.5 * (dmax ** 2 - dmin ** 2)

        if max(f_v_1, f_v_2, f_v_3) == f_v_1:
            amin = 0
            bmin = 0
            
        elif max(f_v_1, f_v_2, f_v_3) == f_v_2:
            amin = 1 / (dmax - dmin)
            bmin = (-dmin) / (dmax - dmin)
    ...
    def get_monom_coeffs_layer(self):
        ...
        for interval in self.intervals:   
            ...
            if interval[1] <= 0:
                ...
                coeffs_over = self.get_monom_coeffs_over(interval)
                # coeffs_over = self.get_quadratic_coeffs_over(interval)
                ...
                # coeffs_under = self.get_quadratic_coeffs_under_mod(interval)
