...
def ibf_tensor_prod_input_weights(n_vars, inputs_degrees, inputs, weights, device):
    ...
    # print('inputs[0].device ', inputs[0].device)
...
class NodeModule(torch.nn.Module):
    ...
    def forward(self, layer_inputs_under, layer_inputs_over, layer_inputs_under_degrees, layer_inputs_over_degrees):
        ...
        if self.activation == 'relu':
            ...
            if u <= 0.0:
                return torch.tensor([0.], device=self.device), torch.tensor([0.], device=self.device), torch.zeros(self.n_vars).to(self.device), torch.zeros(self.n_vars).to(self.device)
...
class LayerModule(torch.nn.Module):
    ...
    def forward(self, layer_inputs_under, layer_inputs_over, layer_inputs_under_degrees, layer_inputs_over_degrees):
        ...
        # Divide the layer_size by the number of GPUs to get the batch size per GPU
        batch_size = max(self.layer_size // num_gpus, 1)  # Ensure the batch size is at least 1
        ...
        for batch_idx in range(0, self.layer_size, batch_size):
            # Determine the GPU to use for this batch of nodes
            gpu = batch_idx // batch_size % num_gpus
            device = f'cuda:{gpu}'

            # Move each tensor in the list of inputs to the correct GPU
            layer_inputs_under_gpu = [input_tensor.to(device) for input_tensor in layer_inputs_under]
            layer_inputs_over_gpu = [input_tensor.to(device) for input_tensor in layer_inputs_over]
            layer_inputs_under_degrees_gpu = layer_inputs_under_degrees.to(device)
            layer_inputs_over_degrees_gpu = layer_inputs_over_degrees.to(device)

            # Create a batch of nodes assigned to the current device
            nodes_batch = [NodeModule(self.n_vars, self._layer_weights_pos[i, :], self._layer_weights_neg[i, :], self.layer_biases[i], self.activation, device) 
                           for i in range(batch_idx, min(batch_idx + batch_size, self.layer_size))]

            # Parallel processing of nodes in this batch
            batch_results = torch.nn.parallel.parallel_apply(
                nodes_batch, 
                [(layer_inputs_under_gpu, layer_inputs_over_gpu, layer_inputs_under_degrees_gpu, layer_inputs_over_degrees_gpu) for _ in nodes_batch]
            )

            # Collect the results back to gpu0
            for node_output_under, node_output_over, node_output_under_degree, node_output_over_degree in batch_results:
                results_under.append(node_output_under.to('cuda:0', non_blocking=True))  # Move results back to gpu0
        ...
        # Concatenate results from all GPUs on gpu0
        return results_under, results_over, torch.stack(results_under_degrees).to('cuda:0'), torch.stack(results_over_degrees).to('cuda:0')
