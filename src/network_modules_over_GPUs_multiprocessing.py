...
def node_processing(
        node_weights_pos, node_weights_neg, node_bias, activation,
        layer_inputs_under, layer_inputs_over, layer_inputs_under_degrees, layer_inputs_over_degrees,
        device, results_queue, n_vars):
    """
    This function initializes the NodeModule on the specific device, performs the forward pass, and sends the results
    to the designated GPU for aggregation.
    """
    # Initialize the node on the specific device (GPU)
    node = NodeModule(n_vars, node_weights_pos, node_weights_neg, node_bias, activation, device)
    ...
    # Move the results to the designated GPU for aggregation (e.g., GPU 0)
    results_queue.put((
            node_output_under.to('cuda:0'), node_output_over.to('cuda:0'),
            node_output_under_degree.to('cuda:0'), node_output_over_degree.to('cuda:0')
        ))
...
class LayerModule(torch.nn.Module):
    ...
    def forward(self, layer_inputs_under, layer_inputs_over, layer_inputs_under_degrees, layer_inputs_over_degrees):
        # Process nodes in batches, utilizing all available GPUs in parallel
        ...
        for b in range(num_batches):
            processes = []
            # Use 'spawn' context for the queue creation
            results_queue = mp.get_context("spawn").Queue()

            # Process each node in the batch on a separate GPU
            for i in range(num_gpus):
                device = f'cuda:{i}'  # Assign to GPU i
                node_weights_pos = self._layer_weights_pos[b * num_gpus + i, :].to(device)
                node_weights_neg = self._layer_weights_neg[b * num_gpus + i, :].to(device)
                node_bias = self.layer_biases[b * num_gpus + i].to(device)

                # Initialize the node in the subprocess to ensure it's created on the correct GPU
                p = mp.Process(target=node_processing, args=(
                    node_weights_pos, node_weights_neg, node_bias, self.activation,
                    layer_inputs_under, layer_inputs_over, layer_inputs_under_degrees, layer_inputs_over_degrees,
                    device, results_queue, self.n_vars))
                p.start()
                processes.append(p)

            # Collect results from all processes
            for p in processes:
                node_output_under, node_output_over, node_output_under_degree, node_output_over_degree = results_queue.get()
                results_under.append(node_output_under.clone().to('cuda:0'))
                results_over.append(node_output_over.clone().to('cuda:0'))
                results_under_degrees.append(node_output_under_degree.clone().to('cuda:0'))
                results_over_degrees.append(node_output_over_degree.clone().to('cuda:0'))
                del node_output_under, node_output_over, node_output_under_degree, node_output_over_degree
                p.join()

            # Now that we're done with the queue, close and join it
            results_queue.close()
...
class NetworkModule(torch.nn.Module):
    ...
    def __init__(self, n_vars, network_weights, network_biases, network_size, device='cuda:0'):
        super().__init__()
        self.n_vars = n_vars
        self.device = device  # Default to 'cuda:0' or can be set for multi-GPU setups
    ...
    def forward(self, inputs):
        ...
        for i, layer in enumerate(self.layers):
            ...
            inputs_under, inputs_over, inputs_under_degrees, inputs_over_degrees = layer(
                inputs_under, inputs_over, inputs_under_degrees, inputs_over_degrees
            )
...
if __name__ == "__main__":
    ...
    intervals = torch.tensor(intervals, dtype=torch.float32).to('cuda:0')  # Move intervals to cuda:0
    inputs = generate_inputs(n_vars, intervals, device='cuda:0')  # Start inputs on cuda:0
    network_size = [n_vars, 16]
    network_size = torch.tensor(network_size).to('cuda:0')  # Move network_size to cuda:0
    network_weights = []
    network_biases = []

    # Initialize weights and biases on cuda:0
    for i in range(len(network_size) - 1):
        weights = torch.randn(network_size[i + 1], network_size[i], device='cuda:0')
        biases = torch.randn(network_size[i + 1], 1, device='cuda:0')
        network_weights.append(weights)
        network_biases.append(biases)
  
    time_start = time.time()
    with torch.no_grad():
        # Initialize the network on cuda:0
        network = NetworkModule(n_vars, network_weights, network_biases, network_size, device='cuda:0')
        # Run the network forward pass, starting with inputs on cuda:0
