    ...
    def forward(self, layer_inputs_under, layer_inputs_over, layer_inputs_under_degrees, layer_inputs_over_degrees):
        ...
        for batch_idx in range(0, self.layer_size, batch_size):
            ...
            for i in range(batch_idx, min(batch_idx + batch_size, self.layer_size)):
                ...
                # Move each NodeModule to the correct GPU
                node = self.nodes[i].to(device)

                # Prepare inputs for each node
                node_inputs = (
                    [input_tensor.to(device) for input_tensor in layer_inputs_under],
                    [input_tensor.to(device) for input_tensor in layer_inputs_over],
                    layer_inputs_under_degrees.to(device),
                    layer_inputs_over_degrees.to(device)
                )

                nodes_batch.append(node)
                inputs_batch.append(node_inputs)

            # Use parallel_apply for processing nodes in the batch
            batch_results = torch.nn.parallel.parallel_apply(nodes_batch, inputs_batch)

            # Collect and store the results
            for outputs in batch_results:
                node_output_under, node_output_over, node_output_under_degree, node_output_over_degree = outputs
