...
torch::Tensor ibf_dense_cuda(torch::Tensor poly) {
  ...
  int64_t blocks_x = std::min(target_blocks_x, static_cast<int64_t>(int_pow(2, 31) / ebf_size));
  ...
  AT_DISPATCH_FLOATING_TYPES(poly.type(), "ibf_cuda_minmax", ([&] {
    ibf_dense_cuda_kernel<scalar_t><<<blocks, threads>>>(
